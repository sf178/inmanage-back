...
def get_average(mark, model):
    url = f'https://auto.ru/cars/{mark}/{model}/all/?page='  # Replace with your desired URL
    pages = 5

    headers = '''authority: auto.ru
method: GET
...
'''
    ...
    for page in range(1, pages + 1):
        ...
        for item in listing_items:
            ...
            if price_element:
                ...
                price = price_element.text.strip().replace('Â ', '').replace('â½', '')
    ...
    average_sum = sum(price_list) / len(price_list)
    minimum = min(price_list)
    maximum = max(price_list)

    return int(average_sum), minimum, maximum
